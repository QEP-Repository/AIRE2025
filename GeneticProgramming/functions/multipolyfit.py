...
def multipolyfit(xs, y, deg, full=False, model_out=False, powers_out=False):
    ...
    beta, residuals, _, _ = linalg.lstsq(A, y, rcond=None)
    
    if full:
        return beta, residuals, mk_model(beta, powers), powers
...
def mk_model(beta, powers):
    ...
    def model(*args):
        ...
        if x.shape[1]!=(num_covariates):
            raise ValueError("Expected %d inputs"%num_covariates)
        xs = hstack((ones((x.shape[0], 1), dtype=x.dtype) , x))
        return np.sum(np.array([coeff * np.prod(xs**p, axis=1)
                             for p, coeff in zip(powers, beta)]), axis=0)
    return model
...
class multipoly():
    ...
    def fit(self, x, y):
        ...
        self.coeffs = multipolyfit(x, y, self.deg, full=True)[0]
        self.SSE = multipolyfit(x, y, self.deg,  full=True)[1]
        self.model = multipolyfit(x, y, self.deg,  full=True)[2]
